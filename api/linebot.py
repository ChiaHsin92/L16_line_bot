...
def handle_message(event):
    ...
    if user_msg == "會員專區":
        ...
        user_state[user_id] = "awaiting_date_input"
        line_bot_api.reply_message(
            event.reply_token,
            TextSendMessage(text="📅 請輸入日期查詢，例如：2025-05-01")
        )

    elif re.match(r"^\d{4}[-/]\d{2}[-/]\d{2}$", user_msg):
        if user_state.get(user_id) == "awaiting_date_input":
            ...
            line_bot_api.reply_message(
                event.reply_token,
                TextSendMessage(text="📅 請先點選『📅 日期查詢』按鈕，再輸入日期")
            )

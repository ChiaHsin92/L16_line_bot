...
def handle_message(event):
    ...
    if user_msg == "會員專區":
    ...
    elif re.fullmatch(r"\d{4}[-/]\d{1,2}[-/]\d{1,2}", user_msg.strip()):
        try:
            import datetime
            dt = datetime.datetime.strptime(user_msg.strip().replace("/", "-"), "%Y-%m-%d")
            ...
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text="⚠ 請輸入正確的日期格式（例如 2025-05-01）"))
        except Exception as e:
        ...
        logger.info(f"🚨 進入最後 else，收到的 user_msg = {user_msg}")
        logger.info(f"🧭 使用者目前狀態 = {user_states.get(user_id)}")
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text="❓ 抱歉，我無法理解您的訊息，可以再試一次或點選選單"))

...
def handle_message(event):
    ...
    if user_msg == "會員專區":
        ...
        try:
            ...
            # 檢查是否為日期格式（yyyy-mm-dd 或 yyyy/mm/dd）
            if re.match(r"^\d{4}[-/]\d{2}[-/]\d{2}$", user_msg):
                query_date = user_msg.replace("/", "-")
                sheet = client.open_by_key("1jVhpPNfB6UrRaYZjCjyDR4GZApjYLL4KZXQ1Si63Zyg").worksheet("課程資料")
                records = sheet.get_all_records()
                ...
                if matched and matched.get("圖片1", "").startswith("https"):
                    bubble = {
                        ...
                        "body": {
                            ...
                            "spacing": "sm",
                            "contents": [
                                {"type": "text", "text": matched["名稱"], "weight": "bold", "size": "xl", "wrap": True},
                                {"type": "text", "text": matched["描述"], "size": "sm", "wrap": True, "color": "#666666"}
                            ]
                        }
                    }
            ...
            logger.error(f"處理用戶訊息時發生錯誤：{e}", exc_info=True)
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text=f"⚠ 查詢時發生錯誤：{str(e)}"))
